...
private function EventListenerReturn OnTileDataChanged( Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData )
{
	...
	`LOG(GetFuncName() @ "Running",, 'IRICHL');
	`LOG(GetScriptTrace(),, 'IRICHL');
...
function InitEvacZone(XComGameState_EvacZone EvacZoneState)
{
	...
	`LOG(GetFuncName() @ "Running by Evac State on team" @ EvacZoneState.Team @ EvacZoneState.ObjectID @ "Removed:" @ EvacZoneState.bRemoved,, 'IRICHL');
	`LOG(GetScriptTrace(),, 'IRICHL');
